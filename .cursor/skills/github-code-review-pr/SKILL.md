...
description: Code review a PR with inline comments only (no positives), using GitHub MCP. Use when asked to review a pull request.
license: MIT
...
Optional PR number or branch name. If omitted, resolve: current branch PR → only open PR → latest open PR.
