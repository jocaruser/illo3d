---
name: github-check-code-review
description: Review PR feedback, implement approved changes, commit, and reply to resolved comments. Use when the user wants to address review comments on their PR.
license: MIT
compatibility: Requires GitHub MCP and git.
metadata:
  author: agent-commands
  version: "1.0"
---

Check review comments on a PR, explain and advise on each, implement the approved ones, then commit and reply to each resolved comment.

## Input

Optional PR number. If omitted, resolve in order: (a) PR for current branch, (b) only open PR, (c) latest open PR.
