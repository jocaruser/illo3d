import { create } from 'zustand'

/**
 * Progress of the long-running workbook operation (hydrate/refresh reads,
 * whole-workbook saves). Drives the progress toasts and — when `blocking` —
 * the modal save overlay. Ephemeral by nature: never persisted.
 */
export interface OperationProgress {
  kind: 'load' | 'save'
  blocking: boolean
  message: string
  current: number
  total: number
  sheetName: string
}

interface OperationState {
  operation: OperationProgress | null
  start(kind: 'load' | 'save', options: { total: number; blocking: boolean; message: string }): void
  progress(current: number, sheetName: string): void
  finish(): void
}

export const useOperationStore = create<OperationState>()((set) => ({
  operation: null,

  start: (kind, { total, blocking, message }) =>
    set({ operation: { kind, blocking, message, total, current: 0, sheetName: '' } }),

  progress: (current, sheetName) =>
    set((state) =>
      state.operation === null ? state : { operation: { ...state.operation, current, sheetName } }
    ),

  finish: () => set({ operation: null }),
}))
