...
export function SetupWizard({ onCancel }: SetupWizardProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div
        className="relative max-h-[90vh] w-full max-w-md overflow-auto rounded-lg bg-surface-elevated p-6 shadow-xl"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        {welcomeError ? <p className="mb-4 text-sm text-danger">{welcomeError}</p> : null}
        {busy && screen === 'welcome' ? (
          <p className="mb-4 text-sm text-text-muted">{t('wizard.pickingFolder')}</p>
        ) : null}
        {renderMain()}
      </div>
