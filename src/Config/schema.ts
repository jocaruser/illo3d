/**
 * Canonical workbook schema (v3). One entry per sheet, in the canonical order
 * used for creation, validation, hydration and save.
 *
 ...
 *   - inventory + `colour`
 *
 * `schema.dbml` at the repo root mirrors this file — update both together.
 */
...
export const SHEET_HEADERS: Record<SheetName, readonly string[]> = {
  ...
  tag_links: ['id', 'tag_id', 'entity_type', 'entity_id', 'created_at', 'archived', 'deleted'],
  jobs: [
    'id',
    'client_id',
    'description',
  ...
  piece_items: ['id', 'piece_id', 'inventory_id', 'quantity', 'archived', 'deleted'],
  inventory: [
    'id',
    'type',
    'name',
...
export const DATA_SHEET_NAMES = SHEET_NAMES.filter((name) => name !== 'audit_log')
